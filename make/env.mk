...
ifeq ($(origin NEXT_PUBLIC_USE_COOKIE_SESSION), undefined)
  ifneq ($(strip $(SESSION_COOKIE_ENABLED)),)
    NEXT_PUBLIC_USE_COOKIE_SESSION := $(SESSION_COOKIE_ENABLED)
  endif
endif

MAKE_ENV_EXPORT_VARS ?= \
  PROJECT_ID REGION BUCKET_NAME INDEX_ID INDEX_ENDPOINT_ID DEPLOYED_INDEX_ID \
  CHUNKS_PATH VECTOR_BACKEND LLM_BACKEND \
  SESSION_COOKIE_ENABLED \
  OPS_AUTH OPS_SECRET API_KEY PERSONA_NAME MAX_INPUT_TOKENS MAX_OUTPUT_TOKENS REQ_TIMEOUT_MS \
  WEIGHTED_CONSENSUS_COUNT \
  NEXT_PUBLIC_API_URL NEXT_PUBLIC_USE_COOKIE_SESSION GOOGLE_APPLICATION_CREDENTIALS \
