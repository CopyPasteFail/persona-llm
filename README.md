...
After the deploy, drop the elevated rights:

```bash
make gcp-sa-revoke-builder
```

For automation that needs Application Default Credentials, export the JSON key generated by `make gcp-sa-key` (stored under `private/secrets/key.json` by default) or use Secret Manager to fetch it at runtime:
